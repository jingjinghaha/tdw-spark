...
object Feature2svm {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data_partition = data.filter(
      data("date_key").isNotNull &&
        ...
        data("account_res_score").isNotNull).repartition(10000)
    ...
    val filteredProfile = data_partition.join(userProfileDF, data_partition("uin") === userProfileDF("uin2"),"inner").repartition(10000)//.cache()
    ...
    train_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://ss-sng-dc-v2/data/SPARK/SNG/g_sng_im_g_sng_mediaaccount/franniewu/train_data/svm_train/feature_v4/"+dateStr)
    test_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://ss-sng-dc-v2/data/SPARK/SNG/g_sng_im_g_sng_mediaaccount/franniewu/train_data/svm_test/feature_v4/"+dateStr)
  }
}
