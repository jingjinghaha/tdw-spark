...
object bin24_Feature2svm {
  def main(args:Array[String]): Unit ={
    ...
    //    val data = tdw.table("simsonhe_kandian_train_data", DateUtil.getOneWeek(dateStr))
    //    println(DateUtil.getOneWeek(dateStr))

    val origTrainData = data.filter(
      data("date_key").isNotNull && data("hour").isNotNull && data("city_level").isNotNull && data("stay_time").isNotNull && data("title_length").isNotNull && data("cover_pic").isNotNull && data("quality_score").isNotNull && data("account_score").isNotNull && data("attract_point").isNotNull && data("read_time").isNotNull && data("read_complete").isNotNull && data("title_emotion").isNotNull && data("title_attract").isNotNull && data("channel_click_rate").isNotNull && data("channel_like_rate").isNotNull && data("channel_comment_rate").isNotNull && data("channel_id").isNotNull && data("algorithm_rate").isNotNull && data("algorithm_id").isNotNull && data("uin").isNotNull && data("article_id").isNotNull && data("di_score").isNotNull && data("di_usermodel_score").isNotNull && data("di_tag_score").isNotNull && data("di_hqtag_score").isNotNull && data("account_read_score").isNotNull && data("account_click_score").isNotNull && data("account_like_score").isNotNull && data("account_comment_score").isNotNull && data("account_health_score").isNotNull && data("account_res_score").isNotNull)
    //    println("The number of origTrainData: " + origTrainData.count())    //704494784
    ...
    val userProfileDF = userProfileData.dropDuplicates("uin2").repartition(8000)

    val uinPackage = spark.read.format("csv").load("hdfs://tl-if-nn-tdw:54310/stage/outface/sng/sng_mediaaccount_app/franniewu/neisou_3_5yi_slice_24_19").toDF("uin1")
    val finalData0 = origTrainData.join(uinPackage, uinPackage("uin1") === origTrainData("uin"),"inner").repartition(800)//.cache()
    //    println(finalData0.count())    //21482968
    val finalData1 = finalData0.join(userProfileDF, userProfileDF("uin2") === finalData0("uin"),"inner")//.cache()
    //    println(finalData1.count())    //21482956
    val finalData2 = finalData1.join(article_info, article_info("cms_article_id") === finalData1("article_id")).repartition(800).cache()
    println(finalData2.count())      //14419808
    ...
    val final_output = finalData2
      .withColumn("1stWeight", get1stChannelFunc(col("potrait")))
      .withColumn("2ndWeight", get2ndChannelFunc(col("potrait")))

    val assemblerOutput = new VectorAssembler()
      .setInputCols(Array(
        "hour","age",
        "title_length", "cover_pic",
        "channel_id",
        //"algorithm_id",
        "title_ellipsis",
        "avtive_level", "quality_score", "account_score",
        //"read_time",
        "read_complete", "channel_click_rate",
        //"channel_like_rate", "channel_comment_rate",
        //"algorithm_rate",
    ...
    val Array(train_data, test_data) =  MLUtils.convertVectorColumnsToML(assemblerOutput).randomSplit(Array(0.7, 0.3))
    println(train_data.count(), test_data.count())

    train_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://tl-if-nn-tdw:54310/stage/outface/sng/sng_mediaaccount_app/franniewu/train_data/svm_train/bin24/orig_v4/"+dateStr)
    test_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://tl-if-nn-tdw:54310/stage/outface/sng/sng_mediaaccount_app/franniewu/train_data/svm_test/bin24/orig_v4/"+dateStr)
