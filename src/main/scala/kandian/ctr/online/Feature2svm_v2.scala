...
object Feature2svm_v2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = tdw.table("rerank_train_data_kehuduan", Seq("p_"+dateStr))

    val desiredNum = 230000000
    val dataNum = data.count()
    println("the number of data in "+dateStr+" is: "+dataNum)
    ...
    val articleProfile = tdw.table("haohaochen_article_channel", DateUtil.getOneWeek(dateStr)).toDF("today_date", "cms_article_id","channel_1st","channel_2nd","tag00","tag01","tag02","tag03","tag04","tag05","tag06","tag07","tag08","tag09","tag10","tag11","tag12","tag13","tag14","tag15","tag16","tag17","tag18","tag19")
    ...
    val userProfileDF = userProfileData.dropDuplicates("uin2").repartition(4000)

    val filteredProfile = data_partition.join(userProfileDF, data_partition("uin") === userProfileDF("uin2"),"inner").repartition(4000)//.cache()

    val filteredArticle = filteredProfile.join(filteredArticleProfile, filteredArticleProfile("cms_article_id") === filteredProfile("article_id")).repartition(4000).cache()
    ...
    println(train_data_filter_neg.count(), test_data.count())

    // save final train data and test data to HDFS
    train_data_filter_neg.write.format("libsvm").mode("overwrite")
      .save("hdfs://ss-sng-dc-v2/data/SPARK/SNG/g_sng_im_g_sng_mediaaccount/franniewu/train_data/svm_train/feature_v5/sampled/"+dateStr)
    test_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://ss-sng-dc-v2/data/SPARK/SNG/g_sng_im_g_sng_mediaaccount/franniewu/train_data/svm_test/feature_v5/sampled/"+dateStr)
  }
}
