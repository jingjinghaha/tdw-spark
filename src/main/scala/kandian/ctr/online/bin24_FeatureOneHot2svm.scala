...
/**
  * Created by franniewu on 2017/6/26.
  */
object bin24_FeatureOneHot2svm {
...
//    println(finalData1.count())    //21482956
    ...
    val newData = DataProcessing.transFeatureMapping(finalData2,dateStr).select("label","features").repartition(800)
    newData.show(20,false)

    val Array(train_data, test_data) =  MLUtils.convertVectorColumnsToML(newData).randomSplit(Array(0.7, 0.3))
    train_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://tl-if-nn-tdw:54310/stage/outface/sng/sng_mediaaccount_app/franniewu/train_data/svm_train/bin24/"+dateStr)
    test_data.write.format("libsvm").mode("overwrite")
      .save("hdfs://tl-if-nn-tdw:54310/stage/outface/sng/sng_mediaaccount_app/franniewu/train_data/svm_test/bin24/"+dateStr)
